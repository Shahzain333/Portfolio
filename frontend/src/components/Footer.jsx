...
export default function Footer() {
  ...
  return (
    <footer style={{ borderTop: '1px solid var(--clr-border)', background: 'linear-gradient(180deg, var(--clr-bg-2), rgba(255,255,255,0.18))' }}>
      <div className="container-page">
        <div className="py-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-5 md:gap-8" style={{ paddingBottom: '0.8rem', paddingTop: '0.8rem' }}>
          
          <Logo />

          <nav className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
            {NAV_LINKS.map(([to, label]) => (
              <Link key={to} to={to} className="transition-colors duration-200" style={{ color: 'var(--clr-text-2)' }} onMouseEnter={(e) => { e.currentTarget.style.color = 'var(--clr-primary)' }} onMouseLeave={(e) => { e.currentTarget.style.color = 'var(--clr-text-2)' }}>
                {label}
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-2.5 flex-shrink-0">
            {SOCIALS.map(({ label, href, icon: Icon, isLucide, hoverColor, darkHoverColor }) => {
              const iconEl = isLucide ? <Icon size={16} /> : <Icon size={16} />
              const sharedProps = {
                'aria-label': label,
                title: label,
                className: 'w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-200',
                style: { color: 'var(--clr-text-2)', background: 'var(--clr-bg-3)', border: '1px solid var(--clr-border)' },
        ...
        <div className="py-6 text-center" style={{ paddingBottom: '0.5rem', paddingTop: '0.5rem' }}>
          <p className="text-xs tracking-[0.08em] uppercase" style={{ color: 'var(--clr-text-3)' }}>
