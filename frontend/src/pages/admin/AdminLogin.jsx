...
export default function AdminLogin() {
  ...
  const { isLoggedIn, loading, checkLoading } = useSelector(s => s.auth ?? {})
  ...
  useEffect(() => {
    if (!checkLoading && isLoggedIn) {
      navigate('/admin/dashboard', { replace: true })
    }
  }, [isLoggedIn, checkLoading])
  ...
  // While verifying session on reload — show nothing to prevent form flash
  if (checkLoading) return null

  return (
    <div style={{ background:'var(--clr-bg)', minHeight:'100vh', overflowY:'auto' }}>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="card p-8" style={{ boxShadow:'var(--shadow-lg)' }}>
            ...
            <form onSubmit={onSubmit} className="space-y-5" noValidate>
              ...
              <div>
                <label className="block text-sm font-semibold mb-1.5"
                  style={{ color:'var(--clr-text-2)' }}>Email</label>
                <input type="email" autoComplete="username" placeholder="admin@portfolio.com"
                  ...
                  disabled={loading} />
                {errors.email && (
                  <p className="text-xs mt-1" style={{ color:'var(--clr-error)' }}>{errors.email}</p>
                )}
              </div>
              ...
              <div>
                <label className="block text-sm font-semibold mb-1.5"
                  style={{ color:'var(--clr-text-2)' }}>Password</label>
                ...
                {errors.password && (
                  <p className="text-xs mt-1" style={{ color:'var(--clr-error)' }}>{errors.password}</p>
                )}
              </div>
            ...
            <p className="text-center mt-6 text-sm">
              <Link to="/" style={{ color:'var(--clr-text-2)' }}
