...
const fd = (i) => ({ initial: { opacity: 0, y: 24 }, animate: { opacity: 1, y: 0 }, transition: { delay: i * 0.09, duration: 0.55, ease: [0.22, 0.68, 0, 1.2] } })
    ...
    <div style={{ color: 'var(--clr-text)' }}>
      ...
      <section className="section home-hero relative overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute -top-28 -right-16 w-[620px] h-[620px] rounded-full opacity-[0.10]" style={{ background: 'var(--grad-primary)', filter: 'blur(90px)' }} />
          <div className="absolute -bottom-32 -left-20 w-[420px] h-[420px] rounded-full opacity-[0.08]" style={{ background: '#22d3ee', filter: 'blur(80px)' }} />
        </div>
        ...
        <div className="container-page pt-5 relative z-10">
          ...
          <div className="max-w-4xl">
            ...
            <motion.h1 {...fd(1)} className="font-extrabold tracking-[-0.06em] leading-[0.95] mb-6" style={{ fontSize: 'clamp(3rem, 7vw, 6rem)' }}>
              Building
              <span className="gradient-text block">digital experiences</span>
            </motion.h1>

            <motion.p {...fd(2)} className="leading-relaxed mb-10 max-w-2xl" style={{ fontSize: 'clamp(1.05rem, 2vw, 1.26rem)', color: 'var(--clr-text-2)' }}>
              I design and build fast, thoughtful, production-ready web products that blend clean code, clear UX, and real business value.
            </motion.p>

            <motion.div {...fd(3)} className="flex flex-wrap gap-3 mb-10">
            ...
            <motion.div {...fd(4)} className="grid gap-4 sm:grid-cols-3 max-w-3xl">
              {[
                { icon: Layers, val: projects.length, label: 'Projects Built' },
                { icon: Code2, val: skills.length, label: 'Technologies' },
                { icon: Briefcase, val: experiences.length, label: 'Work Experience' },
              ].map(({ icon: Icon, val, label }) => (
                <div key={label} className="card p-4" style={{ boxShadow: 'var(--shadow-card)' }}>
                  <div className="flex items-center gap-3">
                    <div className="w-11 h-11 rounded-xl flex items-center justify-center" style={{ background: 'rgba(79, 70, 229, 0.1)' }}>
                    ...
                    <div>
                      ...
                      <div className="text-xs mt-1 uppercase tracking-[0.12em]" style={{ color: 'var(--clr-text-3)' }}>{label}</div>
                    </div>
                  </div>
                </div>
              ))}
      ...
      <section className="section-sm" style={{ background: 'var(--clr-bg-2)' }}>
        <div className="container-page">
          <div className="grid sm:grid-cols-3 gap-5">
            {[
              { icon: Zap, title: 'Fast & Performant', desc: 'Optimized code and thoughtful architecture for quicker experiences and better UX.' },
              { icon: Shield, title: 'Secure & Reliable', desc: 'Robust validation and clean implementation patterns built for long-term stability.' },
              { icon: Sparkles, title: 'Clean & Scalable', desc: 'Readable, maintainable systems designed to grow with product and team needs.' },
            ].map(({ icon: Icon, title, desc }) => (
              <div key={title} className="card p-6 text-center card-hover">
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: 'rgba(79, 70, 229, 0.08)' }}>
                  <Icon size={22} style={{ color: 'var(--clr-primary)' }} />
                </div>
                <h3 className="font-bold mb-2 text-lg" style={{ color: 'var(--clr-text)' }}>{title}</h3>
          ...
          {pLoad ? <Loader /> : projects.length === 0 ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {projects.slice(0, 3).map((p, i) => {
                ...
                return (
                  <motion.div key={p._id} {...fd(i)} className="card group overflow-hidden card-hover">
          ...
          <div className="container-page">
            <div className="flex items-end justify-between mb-8 gap-4">
              <div>
                ...
                <h2 className="section-title" style={{ fontSize: '1.9rem' }}>Skills & Tools</h2>
              </div>
              <Link to="/skills" className="btn btn-ghost btn-sm">All Skills <ArrowRight size={14} /></Link>
            </div>
            <div className="flex flex-wrap gap-2.5">
              {skills.map((s) => (
                <div key={s._id} className="flex items-center gap-2 px-4 py-2.5 rounded-xl transition-all duration-200" style={{ background: 'var(--clr-bg-card)', border: '1px solid var(--clr-border)', color: 'var(--clr-text-2)', fontSize: '.875rem', fontWeight: 600, cursor: 'default' }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--clr-primary)'; e.currentTarget.style.color = 'var(--clr-primary)'; e.currentTarget.style.boxShadow = 'var(--shadow-sm)'; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--clr-border)'; e.currentTarget.style.color = 'var(--clr-text-2)'; e.currentTarget.style.boxShadow = 'none'; }}>
